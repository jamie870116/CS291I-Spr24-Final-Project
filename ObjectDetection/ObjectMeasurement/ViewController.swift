...
//            guard let results = yoloRequest.results else { return }
            ...
            print("The detected object is \(result.labels.first!.identifier)")
    ...
    func renderer(_ renderer: SCNSceneRenderer, updateAtTime time: TimeInterval) {
        ...
        if let camera = sceneView.pointOfView {
            bgNode.position = camera.convertPosition(position, to: nil)
            bgNode.eulerAngles = camera.eulerAngles
...
//        node.addChildNode(plane)
    ...
    func session(_ session: ARSession, didUpdate frame: ARFrame) {
        ...
        distanceFromDeviceLabel.text = "This is the distance from camera \(distance)"
        
        let pixelBuffer = frame.capturedImage
        detectObject(pixelBuffer: pixelBuffer,frame: frame)
        let camera = frame.camera
            let screenPosition = CGPoint(x: 0.5, y: 0.5)
